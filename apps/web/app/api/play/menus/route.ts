...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!v || v.tier !== "pro") return NextResponse.json({ menus: [], anonymous: true });

    // Anonymous visitor (no session, or session without a customer record at
    // this venue) → still show PUBLIC ("all") campaign menus as a conversion
    // hook ("here's today's deal — sign up to play"). Segment-targeted menus
    // require knowing the customer, so they stay hidden until login.
    ...
    if (user) {
      ...
      if (c) {
        ...
        // Higher tiers inherit lower-tier audiences.
    ...
    if (menus.length === 0) return NextResponse.json({ menus: [], anonymous: !isMember });
    ...
    return NextResponse.json({ menus: result, anonymous: !isMember });
  } catch (e: unknown) {
